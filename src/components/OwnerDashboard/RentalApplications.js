  ...
  useEffect(() => {
    ...
    const fetchApplications = async () => {
      ...
      try {
        ...
        const response = await axios.get('http://localhost:5162/api/RentalApplication/owner', {
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    ...
    try {
      const response = await axios.put(
        ...
        newStatus, // Send the status directly as the request body
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json' // Update Content-Type to text/plain
          }
        }
      );
        ...
        applications.map((app) => (
          <div key={app.rentalApplicationID} className="application-card">
            ...
            <div className="action-buttons">
              <button onClick={() => handleStatusUpdate(app.rentalApplicationID, 'Approved')} className="accept-btn">
